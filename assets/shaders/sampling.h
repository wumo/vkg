#ifndef VKG_SAMPLING_H
#define VKG_SAMPLING_H
...
vec2 rnd2(inout uint prev) { return vec2(rnd(prev), rnd(prev)); }
...
vec3 environment_sample(
  sampler2D lat_long_tex, sampler2D sample_buffer, inout uint seed, out vec3 to_light,
  out float pdf) {
...
vec3 offsetRayUE5(in vec3 origin, in vec3 worldNormal, in vec3 rayDir) {
  const float minBias = 0.01;
  const float maxBias = 0.05;
  const float normalBias = mix(maxBias, minBias, clamp(dot(worldNormal, rayDir), 0, 1));
  return origin + normalBias * worldNormal;
}
