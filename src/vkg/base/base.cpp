...
Base::Base(const WindowConfig &windowConfig, const FeatureConfig &featureConfig): featureConfig_{featureConfig} {
    ...
    swapchain_ = std::make_unique<Swapchain>(*device_, window_->vkSurface(), featureConfig_);
    createSyncObjects();
...
auto Base::createDebugUtils() -> void {
    ...
    callback = instance->vkInstance().createDebugUtilsMessengerEXTUnique(createInfo, nullptr);
#endif
}
...
auto Base::createCommandBuffers() -> void {
    auto _device = device_->vkDevice();
    auto numFrames = uint32_t(device_->queues().size());
    vk::CommandBufferAllocateInfo info{device_->cmdPool(), vk::CommandBufferLevel::ePrimary, numFrames};
...
void Base::onFrame(uint32_t imageIndex, float elapsed) {
    ...
    image::setLayout(
        cb, image, vk::ImageLayout::eUndefined, vk::ImageLayout::ePresentSrcKHR, vk::AccessFlagBits::eTransferWrite,
...
auto Base::syncSemaphore(double elapsed, const std::function<void(uint32_t, double)> &updater) -> void {
    auto &sync = semaphoreSyncs[frameIndex];

    auto resourcesFence = *sync.resourcesFence;
    device_->vkDevice().waitForFences(resourcesFence, true, std::numeric_limits<uint64_t>::max());
...
auto Base::syncTimeline(double elapsed, const std::function<void(uint32_t, double)> &updater) -> void {
    ...
    std::vector<vk::Semaphore> waitSemaphores{sync.semaphore.get(), sync.wsiImageAvailable.get()};
    std::array<uint64_t, 2> waitValues{lastRenderFinishedValue, 0};

    std::vector<vk::Semaphore> signalSemaphores{sync.semaphore.get(), sync.wsiReadyToPresent.get()};
