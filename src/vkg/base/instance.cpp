...
Instance::Instance(FeatureConfig featureConfig) {
  ...
  if(featureConfig.rayTracing) {
    ...
    // optix requirements
    if(
      allExtensions.contains(VK_KHR_EXTERNAL_MEMORY_CAPABILITIES_EXTENSION_NAME) &&
  ...
  errorIf(
    true,
    "GPU Assisted Validation and Debug Printf shouldn't be enabled at the same time");
  #endif
...
auto Instance::supported() const -> const Instance::SupportedExtension & {
  return supported_;
}
