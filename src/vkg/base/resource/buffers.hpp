...
auto hostCoherentBuffer(
  Device &device, const vk::BufferUsageFlags &usage, vk::DeviceSize sizeInBytes,
  VmaMemoryUsage memoryUsage, const std::string &name = "hostCoherentBuffer")
...
auto devIndexBuffer(
  Device &device, vk::DeviceSize sizeInBytes, const std::string &name = "devIndexBuffer")
  -> std::unique_ptr<Buffer>;
auto hostIndexBuffer(
  Device &device, vk::DeviceSize sizeInBytes, const std::string &name = "hostIndexBuffer")
  -> std::unique_ptr<Buffer>;
auto hostOnlyIndexBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostOnlyIndexBuffer") -> std::unique_ptr<Buffer>;
auto devIndexStorageBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "devIndexStorageBuffer") -> std::unique_ptr<Buffer>;
auto hostIndexStorageBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostIndexStorageBuffer") -> std::unique_ptr<Buffer>;
auto hostOnlyIndexStorageBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostOnlyIndexStorageBuffer") -> std::unique_ptr<Buffer>;

auto devVertexBuffer(
  Device &device, vk::DeviceSize sizeInBytes, const std::string &name = "devVertexBuffer")
  -> std::unique_ptr<Buffer>;
auto hostVertexBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostVertexBuffer") -> std::unique_ptr<Buffer>;
auto hostOnlyVertexBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostOnlyVertexBuffer") -> std::unique_ptr<Buffer>;
auto devVertexStorageBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "devVertexStorageBuffer") -> std::unique_ptr<Buffer>;
auto hostVertexStorageBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostVertexStorageBuffer") -> std::unique_ptr<Buffer>;
auto hostOnlyVertexStorageBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostOnlyVertexStorageBuffer") -> std::unique_ptr<Buffer>;

auto devUniformBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "devUniformBuffer") -> std::unique_ptr<Buffer>;
auto hostUniformBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostUniformBuffer") -> std::unique_ptr<Buffer>;
auto hostOnlyUniformBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostOnlyUniformBuffer") -> std::unique_ptr<Buffer>;

auto devStorageBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "devStorageBuffer") -> std::unique_ptr<Buffer>;
auto hostStorageBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostStorageBuffer") -> std::unique_ptr<Buffer>;
auto hostOnlyStorageBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostOnlyStorageBuffer") -> std::unique_ptr<Buffer>;

auto devIndirectBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "devIndirectBuffer") -> std::unique_ptr<Buffer>;
auto devIndirectStorageBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "devIndirectStorageBuffer") -> std::unique_ptr<Buffer>;
auto hostIndirectBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostIndirectBuffer") -> std::unique_ptr<Buffer>;
auto hostIndirectStorageBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostIndirectStorageBuffer") -> std::unique_ptr<Buffer>;
auto hostOnlyIndirectBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostOnlyIndirectBuffer") -> std::unique_ptr<Buffer>;
...
auto hostRayTracingBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostRayTracingBuffer") -> std::unique_ptr<Buffer>;
auto hostOnlyRayTracingBuffer(
  Device &device, vk::DeviceSize sizeInBytes,
  const std::string &name = "hostOnlyRayTracingBuffer") -> std::unique_ptr<Buffer>;
