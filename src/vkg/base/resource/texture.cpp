...
namespace vkg {
Texture::Texture(Device &device, vk::ImageCreateInfo info, VmaAllocationCreateInfo allocInfo, const std::string &name) {
    ...
    debugLog("allocate image: ", name, " ", VkImage(vmaImage->image), "[", alloc.deviceMemory, "+", alloc.offset, "]");
...
auto Texture::barrier(
    vk::ImageLayout newLayout, const vk::AccessFlags &dstAccess, vk::PipelineStageFlagBits dstStage,
    uint32_t srcQueueFamilyIndex, uint32_t dstQueueFamilyIndex, vk::ImageAspectFlagBits aspect)
    -> vk::ImageMemoryBarrier {
    vk::ImageMemoryBarrier barrier_{
        srcAccess,           dstAccess,           currentLayout,   newLayout,
        srcQueueFamilyIndex, dstQueueFamilyIndex, vmaImage->image, {aspect, 0, info.mipLevels, 0, info.arrayLayers}};
    recordLayout(newLayout, dstAccess, dstStage);
    return barrier_;
}
void Texture::setImageView(vk::ImageViewType viewType, const vk::ImageAspectFlags &aspectMask) {
...
auto Texture::recordLayout(vk::ImageLayout layout, vk::AccessFlags access, vk::PipelineStageFlags stage) -> void {
