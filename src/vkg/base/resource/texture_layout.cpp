...
void setLayout(
    ...
    vk::AccessFlags srcAccessMask, vk::AccessFlags dstAccessMask, vk::PipelineStageFlags srcStageMask,
    vk::PipelineStageFlags dstStageMask, uint32_t baseMipLevel, uint32_t levelCount, uint32_t baseArrayLayer,
    uint32_t layerCount) {
    ...
    cb.pipelineBarrier(srcStageMask, dstStageMask, {}, nullptr, nullptr, imageMemoryBarrier);
}
...
auto transitTo(
    ...
    setLayout(
        cb, texture.image(), texture.layout(), newLayout, texture.accessFlag(), dstAccess, texture.stageFlag(),
        dstStage, 0, texture.mipLevels(), 0, texture.arrayLayers());
