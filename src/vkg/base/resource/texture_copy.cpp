...
auto copy(vk::CommandBuffer cb, vk::Image dstImage, Texture &srcTexture) -> void {
  ...
  cb.copyImage(
    srcTexture.image(), vk::ImageLayout::eTransferSrcOptimal, dstImage,
    vk::ImageLayout::eTransferDstOptimal, region);
}
auto copy(
  vk::CommandBuffer cb, Texture &dstTexture, vk::ImageSubresource subresource,
  vk::Extent3D extent, vk::Buffer buffer, uint32_t offset) -> void {
  transitTo(
    cb, dstTexture, vk::ImageLayout::eTransferDstOptimal,
    vk::AccessFlagBits::eTransferWrite, vk::PipelineStageFlagBits::eTransfer);
  vk::BufferImageCopy region;
  region.bufferOffset = offset;
  region.imageSubresource = {
    subresource.aspectMask, subresource.mipLevel, subresource.arrayLayer, 1};
  region.imageExtent = extent;
  cb.copyBufferToImage(
    buffer, dstTexture.image(), vk::ImageLayout::eTransferDstOptimal, region);
}
