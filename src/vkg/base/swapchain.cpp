...
namespace vkg {
auto chooseSurfaceFormat(const std::vector<vk::SurfaceFormatKHR> &formats) -> vk::SurfaceFormatKHR {
...
auto choosePresentMode(const std::vector<vk::PresentModeKHR> &presentModes, bool vsync) -> vk::PresentModeKHR {
...
auto chooseExtent(uint32_t width, uint32_t height, const vk::SurfaceCapabilitiesKHR &capabilities) -> vk::Extent2D {
    if(capabilities.currentExtent.width != std::numeric_limits<uint32_t>::max()) return capabilities.currentExtent;
    vk::Extent2D extent;
    extent.width = std::max<uint32_t>(
        capabilities.minImageExtent.width, std::min<uint32_t>(capabilities.maxImageExtent.width, width));
    extent.height = std::max<uint32_t>(
        capabilities.minImageExtent.height, std::min<uint32_t>(capabilities.maxImageExtent.height, height));
    return extent;
}

Swapchain::Swapchain(Device &device, vk::SurfaceKHR surface, const FeatureConfig &featureConfig)
    : device{device},
...
auto Swapchain::resize(uint32_t width, uint32_t height, bool vsync) -> void {
    ...
    info.imageUsage = vk::ImageUsageFlagBits::eColorAttachment | vk::ImageUsageFlagBits::eStorage |
    ...
    errorIf(images.size() != imageCount_, "newly created swapchain's imageCount != old imageCount!");
...
auto Swapchain::acquireNextImage(vk::Semaphore imageAvailable, uint32_t &imageIndex) -> vk::Result {
    return vkDevice.acquireNextImageKHR(
        *swapchain, std::numeric_limits<uint64_t>::max(), imageAvailable, vk::Fence(), &imageIndex);
}

auto Swapchain::present(uint32_t frameIdx, uint32_t imageIdx, vk::Semaphore renderFinished) -> vk::Result {
...
auto Swapchain::imageView(uint32_t index) const -> vk::ImageView { return *imageViews[index]; }
