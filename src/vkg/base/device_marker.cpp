...
void Device::name(vk::ImageView object, const std::string &markerName) {
    name(uint64_t(VkImageView(object)), vk::DebugReportObjectTypeEXT::eImageView, markerName);
}
...
void Device::name(vk::DescriptorSet object, const std::string &markerName) {
    name(uint64_t(VkDescriptorSet(object)), vk::DebugReportObjectTypeEXT::eDescriptorSet, markerName);
}

void Device::name(uint64_t object, vk::DebugReportObjectTypeEXT objectType, const std::string &markerName) {
    if(VULKAN_HPP_DEFAULT_DISPATCHER.vkDebugMarkerSetObjectNameEXT) {
        ...
        VULKAN_HPP_DEFAULT_DISPATCHER.vkDebugMarkerSetObjectNameEXT(
            VkDevice(*device_), reinterpret_cast<const VkDebugMarkerObjectNameInfoEXT *>(&nameInfo));
    }
}

void Device::tag(
    uint64_t object, vk::DebugReportObjectTypeEXT objectType, uint64_t name, size_t tagSize, const void *tag) {
    if(VULKAN_HPP_DEFAULT_DISPATCHER.vkDebugMarkerSetObjectTagEXT) {
        ...
        VULKAN_HPP_DEFAULT_DISPATCHER.vkDebugMarkerSetObjectTagEXT(
            VkDevice(*device_), reinterpret_cast<const VkDebugMarkerObjectTagInfoEXT *>(&tagInfo));
    }
}

void Device::begin(vk::CommandBuffer commandBuffer, const std::string &markerName, std::array<float, 4> color) {
    if(VULKAN_HPP_DEFAULT_DISPATCHER.vkCmdDebugMarkerBeginEXT) {
        ...
        VULKAN_HPP_DEFAULT_DISPATCHER.vkCmdDebugMarkerBeginEXT(
            VkCommandBuffer(commandBuffer), reinterpret_cast<const VkDebugMarkerMarkerInfoEXT *>(&markerInfo));
    }
}

void Device::insert(vk::CommandBuffer commandBuffer, const std::string &markerName, std::array<float, 4> color) {
