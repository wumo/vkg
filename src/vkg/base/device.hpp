...
public:
    ...
    void name(uint64_t object, vk::DebugReportObjectTypeEXT objectType, const std::string &markerName);
    void tag(uint64_t object, vk::DebugReportObjectTypeEXT objectType, uint64_t name, size_t tagSize, const void *tag);
    void begin(
        vk::CommandBuffer commandBuffer, const std::string &markerName, std::array<float, 4> color = {1, 1, 1, 1});
    void insert(
        vk::CommandBuffer commandBuffer, const std::string &markerName, std::array<float, 4> color = {1, 1, 1, 1});
...
private:
    ...
    using UniqueAllocator = std::unique_ptr<VmaAllocator, std::function<void(VmaAllocator *)>>;
