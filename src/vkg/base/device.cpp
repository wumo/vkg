...
void executeImmediately(
    ...
    cmdBuffers[0].begin(vk::CommandBufferBeginInfo{vk::CommandBufferUsageFlagBits::eOneTimeSubmit});
    func(cmdBuffers[0]);
    cmdBuffers[0].end();
...
auto chooseBestPerformantGPU(std::vector<vk::PhysicalDevice> &physicalDevices) -> vk::PhysicalDevice {
    ...
    std::priority_queue<vk::PhysicalDevice, std::vector<vk::PhysicalDevice>, decltype(cmp)> queue(cmp);
    ...
    debugLog(
        ...
        ", API: ", prop.apiVersion >> 22u, ".", (prop.apiVersion >> 12u) & 0x3ffu, ".", prop.apiVersion & 0xfffu);
    return chosen;
}
...
void Device::findQueueFamily() {
    ...
    for(; gfxIdx < queueFamilies.size(); gfxIdx++)
        if(auto &family = queueFamilies[gfxIdx]; physicalDevice_.getSurfaceSupportKHR(gfxIdx, surface) &&
...
void checkDeviceExtensionSupport(const vk::PhysicalDevice &device, std::vector<const char *> &requiredExtensions) {
    ...
    for(const auto *required: requiredExtensions) {
        errorIf(!availableExtensions.contains(required), "required device extension:", required, "is not supported!");
        debugLog(required);
    }
}

Device::Device(Instance &instance, vk::SurfaceKHR surface, const FeatureConfig &featureConfig)
    ...
    auto features12 = physicalDevice_.getFeatures2<vk::PhysicalDeviceFeatures2, vk::PhysicalDeviceVulkan12Features>()
                          .get<vk::PhysicalDeviceVulkan12Features>();

    *pNext = &features12;
    pNext = &features12.pNext;

    errorIf(!features12.scalarBlockLayout, "required feature scalarBlockLayout not supported!");
    errorIf(!features12.drawIndirectCount, "required feature drawIndirectCount not supported!");
    errorIf(!features12.timelineSemaphore, "required feature timelineSemaphore not supported!");
    ...
    std::vector<vk::DeviceQueueCreateInfo> queueCreateInfos{{{}, queueFamily_, queueCount, priorities.data()}};
    ...
    cmdPool_ = device_->createCommandPoolUnique(
        vk::CommandPoolCreateInfo{{vk::CommandPoolCreateFlagBits::eResetCommandBuffer}, queueFamily_});
...
void Device::execSync(const std::function<void(vk::CommandBuffer)> &func, uint32_t queueIdx, uint64_t timeout) {
...
auto Device::memProps() -> const vk::PhysicalDeviceMemoryProperties & { return memProps_; }
...
auto Device::rayTracingProperties() -> const vk::PhysicalDeviceRayTracingPropertiesNV & { return rtProperties_; }
auto Device::multiviewProperties() -> const vk::PhysicalDeviceMultiviewProperties & { return multiviewProperties_; }
