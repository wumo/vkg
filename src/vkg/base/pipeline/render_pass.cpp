...
auto RenderPassMaker::subpass(vk::PipelineBindPoint bindPoint, uint32_t viewmask)
  -> SubpassMaker & {
...
auto RenderPassMaker::dependency(uint32_t srcSubpass, uint32_t dstSubpass)
  -> DependencyMaker {
...
AttachmentMaker::AttachmentMaker(RenderPassMaker &maker, uint32_t index)
...
SubpassMaker::SubpassMaker(vk::PipelineBindPoint bindpoint, uint32_t index)
...
auto SubpassMaker::color(
  uint32_t colorAttachment, uint32_t resolveAttachment, vk::ImageLayout colorLayout,
  vk::ImageLayout resolveLayout) -> SubpassMaker & {
...
auto SubpassMaker::depthStencil(uint32_t attachment, vk::ImageLayout layout)
  -> SubpassMaker & {
...
SubpassMaker::operator vk::SubpassDescription() const {
  return vk::SubpassDescription(
    ...
    colors.data(), resolve ? resolves.data() : nullptr,
    depth.has_value() ? &depth.value() : nullptr, uint32_t(preserves.size()),
    preserves.data());
}
