...
public:
    ...
    auto vertexInput(std::initializer_list<VertexInputBinding> bindings) -> GraphicsPipelineMaker &;
    auto vertexInputAuto(std::initializer_list<VertexInputAutoBinding> bindings) -> GraphicsPipelineMaker &;
    auto vertexInputBinding(const vk::VertexInputBindingDescription &binding) -> GraphicsPipelineMaker &;
    auto vertexInputAttribute(const vk::VertexInputAttributeDescription &attribute) -> GraphicsPipelineMaker &;

    auto inputAssembly(vk::PrimitiveTopology topology, vk::Bool32 restartEnable = false) -> GraphicsPipelineMaker &;
    ...
    auto shader(vk::ShaderStageFlagBits shaderStage, Shader &&shader) -> GraphicsPipelineMaker &;
