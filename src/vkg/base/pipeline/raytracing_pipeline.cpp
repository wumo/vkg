...
auto RayTracingPipelineMaker::hitGroup(ShaderGroup &&group) -> RayTracingPipelineMaker & {
  ...
  if(!group.closestHit.empty()) {
    ...
    shaders.emplace_back(
      vk::ShaderStageFlagBits::eClosestHitNV, std::move(group.closestHit));
  }
  ...
  if(!group.intersection.empty()) {
    ...
    shaders.emplace_back(
      vk::ShaderStageFlagBits::eIntersectionNV, std::move(group.intersection));
  }
  vk::RayTracingShaderGroupCreateInfoNV groupInfo{
    intersection == VK_SHADER_UNUSED_NV ?
...
auto RayTracingPipelineMaker::addShaderAndGroup(
  vk::ShaderStageFlagBits stage, Shader &&shader) -> void {
...
auto RayTracingPipelineMaker::maxRecursionDepth(uint32_t maxRecursionDepth)
  -> RayTracingPipelineMaker & {
...
auto RayTracingPipelineMaker::createUnique(
  ...
  for(uint32_t g = 0; g < groups.size(); ++g) {
    memcpy(
      pData, shaderHandleStorage.data() + g * shaderGroupHandleSize,
