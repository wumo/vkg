...
public:
    auto pushConstant(const vk::PushConstantRange &pushConstantRange) -> PipelineLayoutMaker &;
    ...
    auto update(uint32_t set, vk::DescriptorSetLayout layout, const DescriptorSetLayoutMaker &setLayoutDef)
        -> PipelineLayoutMaker &;
...
public:
    PipelineLayoutUpdater(PipelineLayoutMaker &layoutMaker, const uint32_t set): layoutMaker(layoutMaker), set_(set) {}

    auto operator()(T &def) -> void { layoutMaker.update(set_, def.setLayout(), def.layoutDef()); }
...
protected:
    ...
    auto makeSp(Args &&...args) -> int32_t {
        ...
        specializations.emplace_back(
            uint32_t(spinfo.entries.size()), spinfo.entries.data(), spinfo.data.size(), spinfo.data.data());
        return int32_t(specializations.size() - 1);
    }
    template<typename T, typename... Args>
    auto sp(
        std::vector<vk::SpecializationMapEntry> &spentries, std::vector<std::byte> &spdata, uint32_t &spoffset, T data,
        Args &&...rest) {
    ...
    template<typename T>
    auto sp(
        std::vector<vk::SpecializationMapEntry> &spentries, std::vector<std::byte> &spdata, uint32_t &spoffset,
        T &&data) {
        auto size = sizeof(T);
