...
public:
  ...
  auto sp(Args &&... spValues) {
    ...
    specializationInfo_ = {
      uint32_t(spentries.size()), spentries.data(), spdata.size(), spdata.data()};
  }
...
public:
  ...
  template<typename... Args>
  explicit Shader(std::span<const uint32_t> opcodes, Args &&... spValues)
    : opcodesSpan{opcodes} {
  ...
  Shader(
    std::span<const uint32_t> opcodes, const vk::SpecializationInfo &specializationInfo)
    : opcodesSpan{opcodes} {
    specializationInfo_ = specializationInfo;
  }
  Shader(std::span<const uint32_t> opcodes, vk::SpecializationInfo &specializationInfo)
    : opcodesSpan{opcodes} {
    specializationInfo_ = specializationInfo;
  }
