...
auto DescriptorSetDef::createSet(vk::DescriptorPool pool) -> vk::DescriptorSet {
    ...
    return DescriptorSetMaker().layout(*setLayout_, layoutMaker.variableDescriptorCount()).create(device_, pool)[0];
}
...
auto DescriptorSetDef::layoutDef() const -> const DescriptorSetLayoutMaker & { return layoutMaker; }
auto DescriptorSetDef::createSets(vk::DescriptorPool pool, uint32_t num) -> std::vector<vk::DescriptorSet> {
    errorIf(!device_, "device is null, call init() first");
    errorIf(!setLayout_, "descriptorSetLayout hasn't been created!");
