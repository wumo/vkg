...
void CamFrustumPass::compile(RenderContext &ctx, Resources &resources) {
    if(!init) {
        ...
        for(int i = 0; i < ctx.numFrames; ++i)
            camBuffers[i] = buffer::devStorageBuffer(resources.device, sizeof(Camera::Desc), toString("camBuffer_", i));
    }
...
void CamFrustumPass::execute(RenderContext &ctx, Resources &resources) {
    ...
    cb.pipelineBarrier(
        ...
        vk::PipelineStageFlagBits::eRayTracingShaderKHR | vk::PipelineStageFlagBits::eDrawIndirect, {},
