...
public:
    ...
    void compile(RenderContext &ctx, Resources &resources) override {
        ...
        if(img != frame.img) {
            ...
            frame.toImg = image::make2DTex(
                ...
                vkUsage::eSampled | vkUsage::eStorage | vkUsage::eTransferSrc | vkUsage ::eTransferDst |
                    vkUsage ::eColorAttachment,
                img->format());
    ...
    void execute(RenderContext &ctx, Resources &resources) override {
        ...
        image::transitTo(
            cb, *frame.toImg, vk::ImageLayout::eColorAttachmentOptimal, vk::AccessFlagBits::eColorAttachmentWrite,
        ...
        vk::Viewport viewport{0,    0,   float(frame.toImg->extent().width), float(frame.toImg->extent().height),
                              0.0f, 1.0f};
        cb.setViewport(0, viewport);
        vk::Rect2D scissor{{0, 0}, {frame.toImg->extent().width, frame.toImg->extent().height}};
        cb.setScissor(0, scissor);

        cb.bindDescriptorSets(
            vk::PipelineBindPoint::eGraphics, pipeDef.layout(), pipeDef.set.set(), frame.set, nullptr);
        if(!std::is_empty_v<PushConstant>) {
            updatePushConstant();
            cb.pushConstants<PushConstant>(pipeDef.layout(), vk::ShaderStageFlagBits::eFragment, 0, pushConstant);
        }
