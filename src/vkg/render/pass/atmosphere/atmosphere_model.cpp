...
auto Interpolate(
    const std::vector<double> &wavelengths, const std::vector<double> &wavelength_function, double wavelength)
    -> double {
...
void ComputeSpectralRadianceToLuminanceFactors(
    const std::vector<double> &wavelengths, const std::vector<double> &solar_irradiance, double lambda_power,
...
auto luminanceFromRadiance(double dlambda, const glm::vec3 &lambdas) -> glm::mat4 {
    auto coeff = [dlambda](double lambda, int component) {
        ...
        return static_cast<float>(
            (XYZ_TO_SRGB[component * 3] * x + XYZ_TO_SRGB[component * 3 + 1] * y + XYZ_TO_SRGB[component * 3 + 2] * z) *
            dlambda);
    };
...
auto makeTex(
    Device &device, vk::ImageType type, uint32_t width, uint32_t height, uint32_t depth, vk::Format format,
    const std::string &name) -> std::unique_ptr<Texture> {
    auto texture = std::make_unique<Texture>(
        ...
        vk::ImageCreateInfo{
            ...
            vk::ImageUsageFlagBits::eSampled | vk::ImageUsageFlagBits::eTransferSrc | vk::ImageUsageFlagBits::eStorage,
            vk::SharingMode::eExclusive},
        VmaAllocationCreateInfo{{}, VMA_MEMORY_USAGE_GPU_ONLY}, name);
    texture->setImageView(
        type == vk::ImageType::e2D ? vk::ImageViewType::e2D : vk::ImageViewType::e3D, vk::ImageAspectFlagBits::eColor);
    texture->setSampler(
        {{},
         vk::Filter::eLinear,
         vk::Filter::eLinear,
...
AtmosphereModel::AtmosphereModel(
    Device &device, double sun_angular_radius, double bottom_radius, double top_radius, float length_unit_in_meters,
    glm::vec3 sunDirection, glm::vec3 earthCenter)
    : device{device} {
    ...
    initParameter(
        wavelengths, solar_irradiance, sun_angular_radius, bottom_radius, top_radius, rayleigh_density,
        rayleigh_scattering, mie_density, mie_scattering, mie_extinction, kMiePhaseFunctionG, ozone_density,
        absorption_extinction, ground_albedo, max_sun_zenith_angle, length_unit_in_meters, 15, 1e-5,
        -glm::normalize(sunDirection), earthCenter);
}

AtmosphereModel::AtmosphereModel(
    Device &device, const std::vector<double> &wavelengths, const std::vector<double> &solar_irradiance,
    ...
    const std::vector<DensityProfileLayer> &absorption_density, const std::vector<double> &absorption_extinction,
    const std::vector<double> &ground_albedo, double max_sun_zenith_angle, float length_unit_in_meters,
    unsigned int num_precomputed_wavelengths, float exposure_scale, glm::vec3 sunDirection, glm::vec3 earthCenter)
    : device{device} {
    initParameter(
        wavelengths, solar_irradiance, sun_angular_radius, bottom_radius, top_radius, rayleigh_density,
        rayleigh_scattering, mie_density, mie_scattering, mie_extinction, mie_phase_function_g, absorption_density,
        absorption_extinction, ground_albedo, max_sun_zenith_angle, length_unit_in_meters, num_precomputed_wavelengths,
        exposure_scale, -glm::normalize(sunDirection), earthCenter);
}

auto AtmosphereModel::atmosphereUBO() const -> BufferInfo { return atmosphereUBO_->bufferInfo(); }
...
auto AtmosphereModel::updateSunIntesity(float intensity) -> void { sunUBO_->ptr<SunUniform>()->intensity = intensity; }
...
auto AtmosphereModel::updateEarthCenter(glm::vec3 earthCenter) -> void {
    sunUBO_->ptr<SunUniform>()->earth_center = {earthCenter, bottom_radius / length_unit_in_meters};
}
...
auto AtmosphereModel::init(uint32_t queueIdx, uint32_t num_scattering_orders) -> void {
    ...
    deltaRayleighScatteringTex = makeTex(
        device, vk::ImageType::e3D, SCATTERING_TEXTURE_WIDTH, SCATTERING_TEXTURE_HEIGHT, SCATTERING_TEXTURE_DEPTH,
        vk::Format::eR32G32B32A32Sfloat, "deltaRayleighScatteringTex");
    deltaMieScatteringTex = makeTex(
        device, vk::ImageType::e3D, SCATTERING_TEXTURE_WIDTH, SCATTERING_TEXTURE_HEIGHT, SCATTERING_TEXTURE_DEPTH,
        vk::Format::eR32G32B32A32Sfloat, "deltaMieScatteringTex");
    deltaScatteringDensityTex = makeTex(
        device, vk::ImageType::e3D, SCATTERING_TEXTURE_WIDTH, SCATTERING_TEXTURE_HEIGHT, SCATTERING_TEXTURE_DEPTH,
        vk::Format::eR32G32B32A32Sfloat, "deltaScatteringDensityTex");
...
auto AtmosphereModel::compute(uint32_t queueIdx, uint32_t num_scattering_orders) -> void {
    ...
    atmosphereUBO_->ptr<AtmosphereUniform>()->atmosphere = calcAtmosphereParams({kLambdaR, kLambdaG, kLambdaB});

    device.execSync([&](vk::CommandBuffer cb) { recordTransmittanceCMD(cb); }, queueIdx);
}

auto AtmosphereModel::precompute(
    uint32_t queueIdx, const glm::vec3 &lambdas, const glm::mat4 &luminance_from_radiance, bool cumulate,
    ...
    device.execSync([&](vk::CommandBuffer cb) { recordTransmittanceCMD(cb); }, queueIdx);
    device.execSync([&](vk::CommandBuffer cb) { recordDirectIrradianceCMD(cb, _cumulate); }, queueIdx);
    device.execSync(
        [&](vk::CommandBuffer cb) { recordSingleScatteringCMD(cb, luminance_from_radiance, _cumulate); }, queueIdx);

    for(auto scatteringOrder = 2u; scatteringOrder <= num_scattering_orders; ++scatteringOrder) {
        device.execSync([&](vk::CommandBuffer cb) { recordScatteringDensityCMD(cb, scatteringOrder); }, queueIdx);
        device.execSync(
            [&](vk::CommandBuffer cb) {
            ...
            queueIdx);
        device.execSync(
            [&](vk::CommandBuffer cb) { recordMultipleScatteringCMD(cb, luminance_from_radiance); }, queueIdx);
    }
}
auto AtmosphereModel::initParameter(
    const std::vector<double> &wavelengths, const std::vector<double> &solar_irradiance, double sun_angular_radius,
    double bottom_radius_, double top_radius, const std::vector<DensityProfileLayer> &rayleigh_density,
    const std::vector<double> &rayleigh_scattering, const std::vector<DensityProfileLayer> &mie_density,
    const std::vector<double> &mie_scattering, const std::vector<double> &mie_extinction, double mie_phase_function_g,
    const std::vector<DensityProfileLayer> &absorption_density, const std::vector<double> &absorption_extinction,
    const std::vector<double> &ground_albedo, double max_sun_zenith_angle, float length_unit_in_meters_,
    unsigned int num_precomputed_wavelengths, float exposure_scale_, glm::vec3 sunDirection, glm::vec3 earthCenter)
    -> void {
    ...
    {
        {
            ...
            atmosphereUBO_ = buffer::hostUniformBuffer(device, sizeof(AtmosphereUniform), "atmosphereUBO");
        ...
        {
            auto spectrum = [=](const std::vector<double> &v, const glm::vec3 &lambdas, double scale) {
            ...
            auto density_layer = [=](const DensityProfileLayer &layer) {
                return DensityProfileLayer{
                    layer.width / length_unit_in_meters_, layer.exp_term, layer.exp_scale * length_unit_in_meters_,
        ...
        }
        {
            transmittanceTex_ = makeTex(
                device, vk::ImageType::e2D, TRANSMITTANCE_TEXTURE_WIDTH, TRANSMITTANCE_TEXTURE_HEIGHT, 1,
                vk::Format::eR32G32B32A32Sfloat, "transmittanceTex");
            ...
            irradianceTex_ = makeTex(
                device, vk::ImageType::e2D, IRRADIANCE_TEXTURE_WIDTH, IRRADIANCE_TEXTURE_HEIGHT, 1,
                vk::Format::eR32G32B32A32Sfloat, "irradianceTex");
        }
    }
}
}