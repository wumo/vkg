...
struct FrameGraphResource: FrameGraphBaseResource {
    ...
    FrameGraphResource(uint32_t id = ~0u, uint32_t revision = 0): FrameGraphBaseResource{id, revision} {}
};
...
public:
    ...
    void enableIf(PassCondition &&passCondition) { passCondition_ = std::move(passCondition); }
...
public:
    ...
    void compile(RenderContext &ctx, Resources &resources) override { compile_(ctx, resources); }
    void execute(RenderContext &ctx, Resources &resources) override { exec_(ctx, resources); }
...
public:
   ...
   */
    ...
    template<typename PassInType, typename PassOutType>
    auto addPass(const std::string &name, const PassInType &inputs, Pass<PassInType, PassOutType> &pass)
        -> Pass<PassInType, PassOutType> &;

    template<typename T, typename... Args>
    auto newPass(const std::string &name, const typename T::PassInType &inputs, Args &&...args) -> T &;

    template<typename PassInType, typename PassOutType>
    auto newLambdaPass(
        const std::string &name, const PassInType &inputs, PassSetup<PassInType> &&setup, PassCompile &&compile,
        PassExec &&exec) -> LambdaPass<PassInType, PassOutType> &;
...
public:
    ...
    auto set(FrameGraphResource<T> &resource, T res) -> Resources & {
        ...
        errorIf(
            out == pass->outputs_.end() || out->second.revision != resource.revision, "resource (",
            resRevisions[resource.id].name, "$", resource.id, ":", resource.revision, ") cannot be written by pass [",
            pass->name, "$", pass->id, ":", pass->order, "]");
    ...
    auto get(FrameGraphResource<T> &resource) -> T {
        ...
        errorIf(
            ...
            resRevisions[resource.id].name, "$", resource.id, ":", resource.revision, ") cannot be read by pass [",
            pass->name, "$", pass->id, ":", pass->order, "]");
...
private:
    ...
    template<typename T>
    auto write(const FrameGraphResource<T> &input, uint32_t passId) -> FrameGraphResource<T> {
        check(input);
...
auto PassBuilder::read(T &input) -> void {
    ...
    if(in != inputs_.end()) {
        errorIf(in->second.revision != input.revision, "cannot read different revision of the same resource in a pass");
        return;
    }
...
auto PassBuilder::write(const FrameGraphResource<T> &input) -> FrameGraphResource<T> {
    ...
    if(out != outputs_.end()) { error("cannot write to the same resource twice in a pass"); }
...
template<typename PassInType, typename PassOutType>
auto PassBuilder::addPass(const std::string &name, const PassInType &inputs, Pass<PassInType, PassOutType> &pass)
    -> Pass<PassInType, PassOutType> & {
...
template<typename T, typename... Args>
auto PassBuilder::newPass(const std::string &name, const typename T::PassInType &inputs, Args &&...args) -> T & {
