...
auto PrimitiveBuilder::newPrimitive(PrimitiveTopology topology) -> PrimitiveBuilder & {
    ...
    if(_primitives.empty()) {
        ...
        indexRange = {last.index.endExclusive(), uint32_t(_indices.size()) - last.index.endExclusive()};
        positionRange = {last.position.endExclusive(), uint32_t(_positions.size()) - last.position.endExclusive()};
        normalRange = {last.normal.endExclusive(), uint32_t(_normals.size()) - last.normal.endExclusive()};
...
auto PrimitiveBuilder::currentVertexID() const -> uint32_t {
    return uint32_t(_positions.size()) - (_primitives.empty() ? 0u : _primitives.back().position.endExclusive());
}

auto PrimitiveBuilder::from(
    std::span<Vertex::Position> positions, std::span<Vertex::Normal> normals, std::span<Vertex::UV> uvs,
    std::span<uint32_t> indices) -> PrimitiveBuilder & {
...
void applyTransform(par_shapes_mesh *m, const mat3 &transform, const vec3 translate = {}) {
...
auto PrimitiveBuilder::triangle(const vec3 p1, const vec3 p2, const vec3 p3) -> PrimitiveBuilder & {
...
auto PrimitiveBuilder::rectangle(const vec3 center, const vec3 right, const vec3 up) -> PrimitiveBuilder & {
    ...
    append(_indices, {vertexID, vertexID + 1, vertexID + 2, vertexID, vertexID + 2, vertexID + 3});
    aabb.merge(p1, p2, p3, p4);
    return *this;
}

auto PrimitiveBuilder::box(vec3 center, vec3 x, vec3 y, float half_z) -> PrimitiveBuilder & {
...
auto PrimitiveBuilder::box(glm::vec3 p1, glm::vec3 p2, glm::vec3 up, float width, float height) -> PrimitiveBuilder & {
...
auto PrimitiveBuilder::circle(vec3 center, vec3 z, float R, int segments) -> PrimitiveBuilder & {
    ...
    for(int i = 2; i <= segments; ++i) {
        ...
        append(_normals, {z});
        append(_uvs, {{0.5f + 0.5f * sin((i - 1) * angle), 0.5f + 0.5f * cos((i - 1) * angle)}});
...
auto PrimitiveBuilder::cone(const vec3 center, const vec3 z, const float R, const int segments) -> PrimitiveBuilder & {
    ...
    for(int i = 0; i < segments; ++i) {
        ...
        append(_uvs, {{0.5f, 0.5f}, {0.5f + 0.5f * sin(i * angle), 0.5f + 0.5f * cos(i * angle)}});
...
auto PrimitiveBuilder::cylinder(const vec3 center, const vec3 z, const float R, const bool cap, const int segments)
    -> PrimitiveBuilder & {
    auto right = cross(z, {0, 0, 1});
    if(epsilonEqual(length(right), 0.f, epsilon<float>())) right = {1, 0, 0};
    right = normalize(right) * R;
    ...
    for(int i = 0; i < segments; ++i) {
        ...
        append(
            _indices, {vertexID + 4 * i, vertexID + 4 * i + 1, vertexID + 4 * i + 2, vertexID + 4 * i,
                       vertexID + 4 * i + 2, vertexID + 4 * i + 3});
    }
    if(cap) {
        circle(center, -z, R, segments);
...
auto PrimitiveBuilder::sphere(vec3 center, float R, int nsubd) -> PrimitiveBuilder & {
    ...
    for(auto i = 0; i < mesh->ntriangles; ++i, vid += 3)
        append(_indices, {triangles[vid] + vertexID, triangles[vid + 1] + vertexID, triangles[vid + 2] + vertexID});
    par_shapes_free_mesh(mesh);
    return *this;
}

auto PrimitiveBuilder::axis(const vec3 center, const float length, const float R, float capLength, const int segments)
    -> PrimitiveBuilder & {
    sphere(center, R, 1);
    newPrimitive();
    // axis X: Red
...
auto PrimitiveBuilder::grid(
    const uint32_t nx, const uint32_t ny, const vec3 center, const vec3 x, const vec3 y, const float wx, const float wy)
    -> PrimitiveBuilder & {
    auto _x = normalize(x);
    auto _y = normalize(y);
...
auto PrimitiveBuilder::rectangleLine(vec3 center, vec3 x, vec3 y) -> PrimitiveBuilder & {
    ...
    append(
        _indices,
        {vertexID, vertexID + 1, vertexID + 1, vertexID + 2, vertexID + 2, vertexID + 3, vertexID + 3, vertexID});
    return *this;
}
