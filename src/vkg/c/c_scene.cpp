...
uint32_t SceneNewPrimitive(
    ...
    return scene_->newPrimitive(
        ...
        {(Vertex::UV *)((float *)uvs + uv_offset_float), numUVs}, {indices, numIndices}, *aabb_,
        static_cast<PrimitiveTopology>(topology), perFrame);
}
void SceneNewPrimitives(CScene *scene, CPrimitiveBuilder *builder, bool perFrame, uint32_t *ptrs) {
    auto *scene_ = reinterpret_cast<Scene *>(scene);
    auto primitives = scene_->newPrimitives(*reinterpret_cast<PrimitiveBuilder *>(builder), perFrame);
...
uint32_t SceneNewTextureFromBytes(
    ...
    return scene_->newTexture(
        {(std::byte *)bytes, numBytes}, width, height, image::toVulkanTextureFormat(format), mipmap);
}
...
uint32_t SceneLoadModel(CScene *scene, char *pathBuf, uint32_t pathSize, CMaterialType type) {
    auto *scene_ = reinterpret_cast<Scene *>(scene);
    return scene_->loadModel(std::string(pathBuf, pathSize), static_cast<MaterialType>(type));
}
uint32_t SceneLoadModelFromBytes(CScene *scene, const char *bytes, uint32_t numBytes, CMaterialType type) {
    auto *scene_ = reinterpret_cast<Scene *>(scene);
    return scene_->loadModel({(std::byte *)bytes, numBytes}, static_cast<MaterialType>(type));
}
