...
void DeleteRenderer(CRenderer *renderer) { delete reinterpret_cast<Renderer *>(renderer); }
...
CScene *RendererAddScene(CRenderer *renderer, CSceneConfig sceneConfig, char *nameBuf, uint32_t size) {
    ...
    return reinterpret_cast<CScene *>(&renderer_->addScene(sceneConfig_, std::string{nameBuf, size}));
}
